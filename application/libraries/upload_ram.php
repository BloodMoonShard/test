...
//╚╝╚╝╚╝╚╩╝──╚╝─╚╝─╚═══╩╝╚╩╝──╚╝
    ...
    function upload_file($WhereAndWhat)
    {
        $config['upload_path'] = $this->CI->input->server('DOCUMENT_ROOT') . '/upload_files/' . $WhereAndWhat . '/';
        $config['allowed_types'] = 'gif|jpg|jpeg|png|doc|docx|xlsx|word|xls|text|txt|zip|pdf';
        $config['max_size'] = '0';
        $config['max_width'] = '0';
        $config['max_height'] = '0';
        $nameEx = explode(".", $_FILES['file_resume']['name']);
        $result_name = 'resume-' . time() . '.' . $nameEx[1];
        $result_name = $this->CI->casual_func->rus2translit($result_name);
        $config['file_name'] = $result_name;
        $this->CI->load->library('upload', $config);
        if ($this->CI->upload->do_upload('file_resume')) {
