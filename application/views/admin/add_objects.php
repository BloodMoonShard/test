...
<?php $role = $this->auth->get_user_role(); ?>
...
<?php } ?>
<div class="col-lg-6">
    <form role="form" class="js-form-address" action="<?php if (isset($id_category)) {
        echo $id_category;
    } ?>" method="post" onkeypress="if(event.keyCode == 13) return false;" enctype="multipart/form-data">
        <div class="form-group">
            <label>Имя объекта</label>
            <input class="form-control" name="name_object" value="<?php if (isset($name_object)) {
                echo $name_object;
            } ?>">
        </div>
        <div class="form-group">
            <label class="text-info" for="status_obj">Статус объекта:</label>
            <?php if($role!=3) { ?>
            <select class="form-control" id="status_obj" name="status_obj">
                <?php
                foreach ($status_options as $so) {
                    $check = '';
                    if (isset($status_obj)) {
                        if ($status_obj == $so['id'])
                            $check = 'selected';
            ...
            <?php }else {
                echo '<p class="form-control disabled">На модерации</p>';
            } ?>
        </div>
        <div class="form-group">
            <label>Артикул</label>
            <input class="form-control" type="text" name="article" value="<?php if (isset($article)) {
                echo $article;
            } ?>">
        </div>
        <div class="form-group">
            ...
            <select class="form-control" name="country">
                <?php foreach ($countries as $cs) { ?>
                    <option <?php if (isset($country) && ($country == $cs['country_id'])) {
                        echo 'selected="selected"';
                    } ?> value="<?= $cs['country_id'] ?>"><?= $cs['name'] ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group">
            <label>Область</label>
            <input class="form-control" type="text" name="region" value="<?php if (isset($region)) {
                echo $region;
            } ?>" id="region">
        </div>
        <div class="form-group">
            <label>Район</label>
            <input class="form-control" type="text" name="district" value="<?php if (isset($district)) {
                echo $district;
            } ?>" id="district">
        </div>
        ...
        <div class="form-group">
            <label>Город</label>
            <input class="form-control" type="text" name="city" value="<?php if (isset($city)) {
                echo $city;
            } ?>" id="id_city">
            <input class="form-control" type="hidden" name="city_id" value="<?php if (isset($city_id)) {
                echo $city_id;
            } ?>" id="id_city_hidden">
        </div>
        <?= $underground; ?>
        ...
        <div class="form-group">
            <label>Улица</label>
            <input class="form-control" type="text" value="<?php if (isset($street)) {
                echo $street;
            } ?>" name="street">
        </div>
        <div class="form-group">
            <label>Дом</label>
            <input class="form-control" type="text" value="<?php if (isset($building)) {
                echo $building;
            } ?>" name="building">
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="type" value="<?php if (isset($type)) {
                ...
                <?php foreach ($type_estate as $te) { ?>
                    <option <?php if (isset($type) && ($type == $te['id_objects_type'])) {
                        echo 'selected="selected"';
                    } ?> value="<?= $te['id_objects_type'] ?>"><?= $te['name'] ?></option>
                        ...
                        <div class="col-lg-4">
                            <div class="panel panel-default">
                                <div class="panel-body building_img_small">
                                    ...
                                    <a class="remove_bimg text-danger"
                                       href="/admin/ajax/remove_ob_img/<?php echo $im['id']; ?>"><i
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <a class="btn btn-primary right" id="add_highway" data-toggle="modal" data-target="#myModal3">Добавить
                    направление</a>
            </div>
        </div>
    </div>
