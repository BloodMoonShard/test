...
//        $this->data['objects'] = array_slice($object_finded, $page, $config["per_page"]);
    ...
    public function search_room()
    {
        ...
        $config["base_url"] = base_url() . "search_room";
        $config["per_page"] = $this->session->userdata('per_page');
        $config["uri_segment"] = 2;
        $config['full_tag_open'] = '<div class="pagination">';
        $config['full_tag_close'] = '</div>';
        ...
        $page = ($this->uri->segment(2)) ? $this->uri->segment(2) : 0;
        if(!$this->input->post()){
            $data = unserialize($this->session->userdata('search_room_post'));
            if(!$data){
                $data = array();
            }
        }else{
            ...
            $this->session->set_userdata('search_room_post', serialize($data));
...
//        FILTER
        $data = unserialize($this->session->userdata('search_room'));
        $xz = array();
        if($data){
            //Filter if issue
            foreach ($object_finded as $v) {
...
//        $this->data['objects'] = array_slice($object_finded, $page, $config["per_page"]);
        ...
        $this->render('/web/search_room.php', $this->data);
    }
}
